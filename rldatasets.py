...
class UltrasoundDataLoader(DataLoader):
    ...
    def __next__(self) -> Tuple[str, str]:
        ...
        gt_dict = json.loads(rec["conversations"][1]["value"])

        # convert position from m to cm and round
        pos_cm = [int(round(gt_dict['position'][axis] * 100)) for axis in ['x', 'y', 'z']]
        rot_rounded = [int(round(gt_dict['rotation'][axis])) for axis in ['x', 'y', 'z']]

        ground_truth_answer = f"<reasoning>\nGenerated by expert.\n</reasoning>\n<answer>\n{':'.join(map(str, pos_cm + rot_rounded))}\n</answer>"

        return img_path, ground_truth_answer
